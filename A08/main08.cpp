...
int main(int argc, char* argv[])
{
    ...
    if (myid == 0) {
        ...
        int blk = R / (numprocs - 1) + 1;
        for (int i = 0; i < R; i++) {
            *cont = 0;
            MPI_Send(&cont, 1, MPI_INT, i / blk + 1, 0, MPI_COMM_WORLD);
            MPI_Send(data[i], 5, MPI_FLOAT, i / blk + 1, 1, MPI_COMM_WORLD);
        }
        ...
        while (true) {
            ...
            std::cout << "process " << myid << " receive data " << std::endl;
            prt(message, 5);
        };
    }
    MPI_Finalize();
